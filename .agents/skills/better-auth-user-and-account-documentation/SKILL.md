---
name: better-auth-user-accounts
description: |
  Reference for managing users and accounts using Better Auth. 
  Use this skill when implementing or explaining Better Auth features such as:
  - updating user profile data
  - changing or verifying email addresses
  - changing or setting passwords
  - verifying passwords for sensitive actions
  - deleting user accounts and handling deletion verification
  - listing, linking, or unlinking authentication provider accounts
  - configuring account linking behavior
  - encrypting OAuth tokens using databaseHooks
  - handling OAuth accounts that don't have passwords

  The skill includes API usage patterns, configuration examples for betterAuth(), 
  client methods (authClient.*), and server APIs (auth.api.*).
---
